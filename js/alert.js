define(function (){
	var clickDan = function(DataType){
		...
		if (isTrue) {
			...
			clickJinDanView(DataType);
		}
	}

	var jiangPinResult = function(DataType){
		var str = '';
		if (DataType=='apple'){
	...
	var alertLogin = function(){
		...
		str += '<input class="phoneInput" name="phoneInput" id="phoneInput" type="text" value="1314520000" placeholder="请输入手机号号码">';
		str += '</div>';
	...
	var clickJinDanView = function(DataType){
		var str = '';
		str += '<div class="bg-mask"></div>';
